# OhMyPm 预留交接对象

## 1. 作用

这个文件是 OhMyPm 向 ShitPM 预留的一份最小单向交接对象。

它的含义是：

- 作为后续正式项目接入时的候选输入
- 不是 ShitPM 自己的状态文件
- 不代表 ShitPM 已经接管当前链路

## 2. 当前需求任务

- 以“审批路径扩展”为样例，验证 OhMyPm 最小主链路是否能跑通。

## 3. 当前版本

- `v0.6`

## 4. 已确认事实

- 当前链路是机制验证样例，不是真实项目协作线程。
- 当前样例原型、PRD 和评审记录已经对齐。
- 当前样例不再向 PM 追问任何虚拟业务问题。

## 5. 当前版本方案摘要

- 保留当前样例链路，作为可回放的 OhMyPm 验证基线。
- 样例中的阈值、标签和角色映射继续保留为样例假设，不上升为真实事实。
- 如果后续转入真实项目，应显式开启新的真实协作上下文，而不是直接复用样例假设。

## 6. 当前模块清单

- 审批能力
  - 现有审批主流程
  - 审批路径扩展
- 通知模块

## 7. 工时与排期视角

- 当前样例的工时讨论仍停留在“原有能力复杂度上升”这一层，不是新增独立模块。
- 这份交接对象不隐含任何正式排期承诺。

## 8. 产物路径

- 原型：`output/prototype/prototype-v1/index.html`
- PRD：`output/prd/prd-v1/PRD-v1.md`
- 评审记录：`output/review/review-v1.json`
- OhMyPm 状态：`.ohmypm/status.json`
- OhMyPm 记忆：`.ohmypm/memory.md`

## 9. 非阻塞说明

- 当前样例链路中已经没有面向 PM 的阻塞问题。
- 如果未来要从样例验证切到真实协作，必须显式切换，不能隐式继承。



