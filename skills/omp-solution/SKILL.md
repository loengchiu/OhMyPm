...
- 方案稿生成完成后默认停在当前阶段；除非 PM 明确确认“方案可以进入下一阶段”或明确要求“继续做原型 / 继续写 PRD / 直接跑完整链路”，否则不得自动进入 `omp-proto` 或 `omp-prd`
...
- 方案稿正文默认使用自然标题、短段落和必要列表组织信息，不靠批量加粗标签、小标题套小标题或解释性空话撑结构
- 页面字段、动作、约束只写当前页面真正影响理解、原型或 PRD 的重点，不把轻页面写成小 PRD
- 页面关键字段下直接列出实际存在的字段类型；存在展示字段就写“展示字段”，存在筛选字段就写“筛选字段”，不存在的字段类型不出现
- 存量承接字段、本次新增字段、待确认口径只在当前页面确实需要区分时出现，不做模板式复读
...
- 当前动作完成后，必须回写：
  - `next_recommended` 默认写成：`下一步：请确认当前方案是否进入原型或 PRD。`
  - `baselines.solution`
