...
- 评审会是正式流程节点，不是普通聊天确认
- 评审开始前必须执行 `scripts/tools/trace-lint.ps1 -StatusPath .ohmypm/status.json`
- 评审开始前必须执行 `scripts/tools/review-pack.ps1 -StatusPath .ohmypm/status.json -OutputPath .ohmypm/review/review-pack.json`
- `trace-lint.ps1` 结果为 `fail` 时不得直接给通过结论，必须先进入修复
