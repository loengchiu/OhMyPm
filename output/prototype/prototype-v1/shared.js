  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (trigger) {
        ...
        if (left > maxLeft) {
          left = rect.left + window.scrollX - cardWidth - 12;
        }
    ...
    document.addEventListener("mousemove", (event) => {
      ...
      const left = Math.max(
        12,
        Math.min(window.innerWidth - dragState.card.offsetWidth - 12, event.clientX - dragState.offsetX)
      );
      const top = Math.max(
        12,
        Math.min(window.innerHeight - dragState.card.offsetHeight - 12, event.clientY - dragState.offsetY)
      );
