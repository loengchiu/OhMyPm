...
- 最小输出：建立当前协作上下文，明确下一步进入先回应还是先补初始化
- 回写：`.ohmypm/status.json` 的当前阶段、最近动作；必要时初始化 `.ohmypm/memory.md`
